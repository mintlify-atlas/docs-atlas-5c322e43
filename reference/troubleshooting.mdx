...
<AccordionGroup>
  <Accordion title='"Failed to fetch" or network error on connect'>
    **Symptom:** Clicking Connect produces an `← error` line in the activity console that includes `"Failed to fetch"` or `"fetch failed"`. The error banner on the page reads something like:
    ...
    **Cause:** The browser is blocking the outgoing MCP request because the target server's HTTP response does not include an `Access-Control-Allow-Origin` header that permits the origin from which Kunna is served. This is enforced by the browser's Cross-Origin Resource Sharing (CORS) policy and cannot be overridden from the client side.
